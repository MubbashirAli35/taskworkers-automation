...
def run_notebook(notebook_name):
    if re.match('[0-9]', notebook[5:6]) == None:
...
# for notebook in backtest_workers:
...
#     if num_of_pending_backtests.iloc[0] > 0:
#         print('Number of Backtests pending: ' + str(num_of_pending_backtests.iloc[0]))
#
#         if num_of_backtests_running.count() > 0:
#             if num_of_backtests_running.iloc[0] < 75:
#                 for i in range(0, 75 - num_of_backtests_running.iloc[0]):
#                     print('python ' + backtests_notebooks_sorted_on_last_beat_time.iloc[i]['Notebook'])
#             else:
#                 print('Max allowed backtest notebooks running')
#         else:
#             if backtests_notebooks_sorted_on_last_beat_time['Notebook'].count() >= 75:
#                 for i in range(75):
#                     print('python ' + backtests_notebooks_sorted_on_last_beat_time.iloc[i]['Notebook'])
#             else:
#                 for i in range(backtests_notebooks_sorted_on_last_beat_time['Notebook'].count()):
#                     print('python ' + backtests_notebooks_sorted_on_last_beat_time.iloc[i]['Notebook'])
